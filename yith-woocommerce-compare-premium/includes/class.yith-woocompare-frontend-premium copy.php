...
if ( ! class_exists( 'YITH_Woocompare_Frontend_Premium' ) ) {
	...
	class YITH_Woocompare_Frontend_Premium extends YITH_Woocompare_Frontend {
		...
		public function add_compare_link( $product_id = false, $args = array() ) {
			...
			printf( '<a href="%s" class="compare %s" data-product_id="%s" rel="nofollow" %s>%s</a>', esc_attr( $link ), esc_attr( $class ), esc_attr( $product_id ), $button_target, esc_html( $button_text ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
